...
import ax.xz.raw.posix.linux.LinuxInterfaceConfigurer;
import ax.xz.raw.posix.osx.OSXInterfaceConfigurer;
...
public interface TunInterfaceConfigurer {
	...
	static TunInterfaceConfigurer get() {
		if (isLinux())
			return new LinuxInterfaceConfigurer();
		else if (isBSD())
			return new OSXInterfaceConfigurer();
		else
